...
xcodebuild -target "${FRAMEWORK_NAME}" -configuration Release -arch arm64 only_active_arch=yes defines_module=yes -sdk "iphoneos" > "$LOGFILE" 2>&1 || {
errormessage=`/sbin/modprobe -n -v hfsplus 2>&1`;
echo $errormessage;
echo " Error in build check log "$LOGFILE"";
exit 1;
}
